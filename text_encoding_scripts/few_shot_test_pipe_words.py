...
def get_task_prediction(challenge_tasks, solutions, task_id, test_input_index) -> List[List]:
    ...
    template=(
            ...
            "The data will be presented as color words separated by '|', and newlines to designate new rows. "
            "Your task is to identify this pattern or transformation and apply it to the test input to get the final output. "
            "Do not give any justification for your answer; just provide the output grid in the same format: color words separated by a pipe '|', with each line representing a row."
            "\n\n{task_string}\n"
    ),
    input_variables=["task_string"]
)
